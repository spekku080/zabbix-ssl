...
if expiration_string=$(whois "$domain" 2>&1 | grep -Ei 'renewal|renewal date|Expiration|Expires on|Expiry date|paid-till|expires' | grep -o -E '[0-9]{4}.[0-9]{2}.[0-9]{2}|[0-9]{2}/[0-9]{2}/[0-9]{4}' | awk 'NR == 1' | sed 's/\./-/g'); then
